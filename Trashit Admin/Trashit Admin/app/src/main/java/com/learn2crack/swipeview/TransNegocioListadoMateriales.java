    ...
    protected String doInBackground(String... strings) {
        try{
            ...
            if (buscar.toString().length() >  0){
                rs1 = st.executeQuery("select * from Material where cast(id as char) like '%"+ buscar +"%'|| Descripcion like '%"+buscar+"%' || Informacion like '%"+buscar+"%' || Imagen like '%"+buscar+"%'");
            }
            else
            {
                rs1 = st.executeQuery("select * from Material");
            }
