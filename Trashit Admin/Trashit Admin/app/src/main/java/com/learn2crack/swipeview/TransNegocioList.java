    ...
    protected String doInBackground(String... strings) {
        ...
        try {
            ...
            if (buscar.toString().length() >  0){
                rs = st.executeQuery("select Puntos_Reciclado.id,direccion,Barrio.descripcion,materiales,geometrycoordinates_y,geometrycoordinates_x from Puntos_Reciclado inner join Barrio on Puntos_Reciclado.barrio = Barrio.id where cast(Puntos_Reciclado.id as char) like '%"+buscar +"%'|| direccion like '%"+buscar+"%' || Barrio.descripcion like '%"+buscar+"%' || materiales like '%"+buscar+"%' || geometrycoordinates_x like '%"+buscar+"%' || geometrycoordinates_y like '%"+buscar +"%' order by Puntos_Reciclado.id asc");
            }
            else
            {
                rs = st.executeQuery("select Puntos_Reciclado.id,direccion,Barrio.descripcion,materiales,geometrycoordinates_y,geometrycoordinates_x from Puntos_Reciclado inner join Barrio on Puntos_Reciclado.barrio = Barrio.id order by Puntos_Reciclado.id asc");
            }
